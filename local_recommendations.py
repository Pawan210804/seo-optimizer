...
def _checks_to_recommendations(checks, max_items):
    ...
    candidates = []
    for check in checks:
        # Not-applicable checks (score None - see rules.py/semantic.py)
        # can't be ranked "weakest first" against real scores, and
        # aren't a real recommendation on their own; get_local_recommendations()
        # adds one combined "add a keyword" note for these instead.
        if check.get("score") is None:
            continue
        fix = check.get("fix", "")
...
def _missing_keyword_recommendation(checks, keyword):
    # If keyword-dependent checks were skipped (score is None - see
    # rules.py/semantic.py), that's real, actionable information: it
    # means part of the report is incomplete, not that those checks
    # passed. Surface it as its own recommendation instead of letting
    # it disappear silently. Only covers checks that are unscored
    # *because* of the missing keyword - "AI Assessment" can also be
    # None for an unrelated reason (no API key configured) and gets
    # its own message from ai_scoring.py instead.
    if keyword:
        return None
    keyword_dependent_names = ("Keyword placement", "Keyword density", "Semantic coverage")
    skipped_names = [c["name"] for c in checks if c.get("score") is None and c["name"] in keyword_dependent_names]
    if not skipped_names:
        return None
    return ("No focus keyword was supplied, so " + ", ".join(skipped_names) + " could not be scored and "
            "were excluded from the overall score. Add a focus keyword for a complete, accurate report.")


def get_local_recommendations(content, checks, keyword_suggestions=None, keyword=None, max_recommendations=10):
    ...
    missing_keyword_line = _missing_keyword_recommendation(checks, keyword)
    slots_for_keyword_line = 1 if keyword_suggestions else 0
    slots_for_missing_keyword = 1 if missing_keyword_line else 0
    recommendations = _checks_to_recommendations(
        checks, max_recommendations - slots_for_keyword_line - slots_for_missing_keyword
    )

    if missing_keyword_line:
        recommendations.insert(0, missing_keyword_line)
