...
def check_semantic_coverage(content, keyword=None):
    if not keyword:
        # Not-applicable rather than a free 100 - a page can't be scored
        # for "on-topic for the keyword" when there is no keyword. See
        # the matching fix in rules.py for keyword placement/density for
        # why this matters: without this, skipping the keyword field was
        # the easiest way to inflate the score, not a neutral no-op.
        return {"name": "Semantic coverage", "score": None, "passed": None, "applicable": False,
                "message": "No focus keyword supplied - semantic/topic coverage cannot be scored.",
                "fix": "Provide a focus keyword to get topic-coverage guidance and a complete score."}
